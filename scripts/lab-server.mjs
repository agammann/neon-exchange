...
const transactions=[];await writeFile('.lab/transactions.json','[]');const token=new Contract(ADDRESSES.USDT,TOKEN_ABI,provider);
...
const allowed=new Set(['eth_chainId','net_version','eth_accounts','eth_blockNumber','eth_getBlockByNumber','eth_getBlockByHash','eth_getBalance','eth_call','eth_estimateGas','eth_gasPrice','eth_maxPriorityFeePerGas','eth_getTransactionCount','eth_getTransactionReceipt','eth_getTransactionByHash','eth_getCode','eth_getLogs','eth_sendTransaction','eth_feeHistory']);
...
const server=createServer(async(req,res)=>{try{
 ...
 if(pathname==='/lab/rpc'){
  ...
  if(method==='eth_sendTransaction'){const tx=params[0];if(!accounts.includes(tx.from.toLowerCase())||![ADDRESSES.ROUTER.toLowerCase(),ADDRESSES.USDT.toLowerCase()].includes(tx.to?.toLowerCase())||Number(tx.chainId)!==31337)throw Error('Only local test account swaps and token approvals are allowed');}
  const result=await fork.request({method,params});
