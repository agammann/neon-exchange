...
for(const mode of ['demo','wallet'])$('mode-'+mode).onclick=()=>{for(const m of ['demo','wallet']){$(m+'-surface').hidden=m!==mode;$('mode-'+m).setAttribute('aria-pressed',String(m===mode));}document.querySelector('.badge').textContent=mode==='demo'?'SIMULATED MARKET':'ETHEREUM MAINNET';};
render();
